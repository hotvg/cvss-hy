...
<body>
<div id="tab-tire" class="parts-tab">
    <ul>
        <li>原装</li>
        <li>加减装</li>
        <li>可换装</li>
    </ul>
    <div id="content-tire"></div>
    <div id="content-tire-add"></div>
    <div id="content-tire-replace"></div>
</div>
<script type="text/javascript">
    var tabTireScript = $('#tab-tire').kendoTabStrip({
        animation: {
            close: {
                duration: 100,
                effects: "fadeOut"
    ...
    tabTireScript.select(0);

    var tireDataSource = new grid.dataSource({
        transport: {
            create: {
                url: "/parts/create",
                dataType: "json"
    ...
    var tireAddDataSource = new grid.dataSource({
        transport: {
            create: {
    ...
    var tireReplaceDataSource = new grid.dataSource({
        transport: {
            create: {
    ...
    var tireReplaceArgs = {
        ...
        dataSource:tireReplaceDataSource,
        dataId: 'replaceId'
    };

    $('#content-tire').dataGrid(tireArgs); //调用jQuery对象方法
    $('#content-tire-add').dataGrid(tireAddArgs);
    $('#content-tire-replace').dataGrid(tireReplaceArgs);
